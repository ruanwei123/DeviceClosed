...
# Using the apply() and shapiro. test() functions to check normality#####
...
#Wilcoxon test for all non normal variables using the Laply function 
...
#Generate baseline three-line table#####
...
legend = paste(colnames(data_train)[1:6], "AUC:", round(auc_list, 3))
...
model158_young_predictions <- ifelse(data_model_young$model158 >= best_threshold_model158, 0, 1)

confusionMatrix(factor(model158_adult_predictions), factor(data_model_adult$Operation))
confusionMatrix(factor(model158_young_predictions), factor(data_model_young$Operation))